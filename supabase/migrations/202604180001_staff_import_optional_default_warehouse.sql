-- Allow staff import commit with no batch-level default warehouse when each row carries warehouse_id.

drop function if exists public.commit_staff_import_batch(uuid, uuid, jsonb);
...
begin
  ...
  loop
    ...
    v_target_wh := coalesce(
      (nullif(trim(elem->>'warehouse_id'), ''))::uuid,
      p_warehouse_id
    );
      ...
      if v_target_wh is null then
        ...
        set
          ...
          validation_errors = jsonb_build_object(
            ...
            'message', 'Each row needs warehouse_code or warehouse_id when no default site is set'
          )
        where batch_id = p_batch_id and row_number = v_row_num;
        continue;
      end if;
